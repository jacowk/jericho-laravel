...
/**
 * Unit test for testing diary item comments for a property flip functionality
 *
 * @author Jaco Koekemoer
 * Date: 2016-11-30
 *
 */
class DiaryItemCommentTest extends TestCase
{
    /**
	 * Test adding a diary item comment and then viewing it
	 */
	public function testAddDiaryItemComment()
	{
		$property_flip_id = $this->createPropertyFlip('Test address line 1000');
		$diary_item_id = $this->createDiaryItem($property_flip_id);
		
		$comment = 'This is a test diary item comment';
		...
		$this->visit('/')
			...
			->press('Add Diary Item Comment')
			->type($comment, 'comment')
			->press('Add Diary Item Comment')
			->see('View Diary Item')
			->see('Diary Item Comment added')
			->see($comment);
	}
	 ...
	 * Test adding a diary item comment and then viewing it
	 */
	public function testAddViewDiaryItemComment()
	{
		$property_flip_id = $this->createPropertyFlip('Test address line 1000');
		$diary_item_id = $this->createDiaryItem($property_flip_id);
		...
		$this->visit('/')
			...
			->click('view-diary-item-comment')
			->see('View Diary Item Comment')
			->see($comment);
	}
	
	/**
	 * Test adding a milestone, then updating it from the Update hyper link on the list of diary item comments
	 */
	public function testAddViewUpdateMilestone()
	{
		...
		$this->visit('/')
			...
			->click('update-diary-item-comment')
			->see('Update Diary Item Comment')
			->type($comment . ' Updated', 'comment')
			->press('Update Diary Item Comment')
			
			->see('Diary Item Comment updated')
			->see('View Diary Item')
			->see($comment . ' Updated');
	}
