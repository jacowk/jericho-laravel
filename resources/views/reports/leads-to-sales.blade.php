...
@section('title')
	Leads To Sales Report
@endsection

@section('content')
	<div class="container">
		<div class="row">
			{{ Form::open(array('route' => 'do-leads-to-sales-report', 'class' => 'form-horizontal')) }}
				{{ Form::token() }}
		...
		<div class="row">
			<div class="panel-heading text-center">
				@if (!empty($properties) && count($properties) > 0)
					<h4 class="panel-title">Leads To Sales Search Result ({{ $properties->total() }} items found)</h4>
				@else
					<h4 class="panel-title">Leads To Sales Search Result</h4>
				@endif
			</div>
		</div>
		<div class="row">
			...
			<div class="text-center">
				@if (!empty($properties) && count($properties) > 0)
					@if ($properties->hasMorePages())
						{{ $properties->appends(['from_date' => $from_date, 'to_date' => $to_date])->render() }}<br/>
					@else
						<ul class="pagination">
							<li><a href="{{ $properties->appends(['from_date' => $from_date, 'to_date' => $to_date])->previousPageUrl() }}" rel="prev">&laquo;</a></li>
							@if ($properties->lastPage() > 10)
								...
								@endfor
								<li><span>...</span></li>
								<li><a href="{{ $properties->appends(['from_date' => $from_date, 'to_date' => $to_date])->url($properties->lastPage() - 1) }}"><span>{{ $properties->lastPage() - 1 }}</span></a></li>
								<li><a href="{{ $properties->appends(['from_date' => $from_date, 'to_date' => $to_date])->url($properties->lastPage()) }}"><span>{{ $properties->lastPage() }}</span></a></li>
							@else
